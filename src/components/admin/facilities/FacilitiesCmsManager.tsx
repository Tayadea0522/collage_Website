  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
        ...
        {/* 1. Facility Name & Description Management */}
        <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-5 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-slate-200/70 pb-3">
            <div className="space-y-0.5">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
                <Edit3 className="w-4 h-4 text-amber-600" /> Facility Title & Name Management
              </label>
              <p className="text-[11px] text-slate-500 font-medium">
                Admin can rename any facility. Changes reflect immediately on public navigation tabs, headers, cards & breadcrumbs.
              </p>
            </div>
            <button
              type="button"
              onClick={handleSaveAll}
              disabled={isSaving || isUploading}
              className="bg-[#0A2342] hover:bg-[#071931] text-amber-400 border border-amber-500/40 text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm transition-all disabled:opacity-50 self-start sm:self-auto"
            >
              <Save className="w-3.5 h-3.5 text-amber-400" />
              {isSaving ? 'Saving...' : 'Save Title to Database'}
            </button>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                placeholder="e.g. Labs, Quality Testing Center, Dairy Processing Plant"
                className="flex-1 text-base font-bold text-[#0A2342] px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white shadow-sm transition-all placeholder:font-normal placeholder:text-slate-400"
              />
              <button
                type="button"
                onClick={() => {
                  const defaultTitle = CORE_CATEGORIES.find(c => c.id === activeCategoryId)?.defaultTitle;
                  if (defaultTitle) handleUpdateFacilityTitle(defaultTitle);
                }}
                className="px-3 py-2 text-xs font-semibold text-slate-600 hover:text-slate-900 bg-white border border-slate-200 hover:bg-slate-100 rounded-xl transition-all whitespace-nowrap"
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(e) => handleUpdateFacilityDescription(e.target.value)}
              placeholder="Facility description displayed on the website..."
              className="w-full text-xs text-slate-700 px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white shadow-sm transition-all placeholder:text-slate-400 resize-none"
            />
          </div>

          <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1 gap-2 border-t border-slate-200/50">
            <span className="flex items-center gap-1">
              <Globe className="w-3.5 h-3.5 text-blue-600" />
              Default system name: <span className="font-mono text-slate-700 font-semibold">{CORE_CATEGORIES.find(c => c.id === activeCategoryId)?.defaultTitle}</span>
            </span>
            <span className="text-emerald-700 font-medium flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
              Live preview updated &bull; Click &quot;Save Title to Database&quot; or &quot;Save All Changes&quot; to publish
            </span>
          </div>
        </div>
